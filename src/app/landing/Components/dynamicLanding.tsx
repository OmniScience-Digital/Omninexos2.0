...
export default function LandingPage() {
    ...
    const [dataArray] = useState<Dashboard[]>([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [dashboardToDelete, setDashboardToDelete] = useState<number | null>(null);

    // Fetch dashboards - ORIGINAL OBSERVEQUERY WITH DEBUG LOGS
    useEffect(() => {
        ...
        const subscription = client.models.Landing.observeQuery().subscribe({
            next: (data) => {
                ...
                console.log('🟢 DEBUG: Clean data:', cleanDashboardData);
                    ...
                    <Card className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm">
                        <CardContent className="p-1 sm:p-2">
                            <div className="space-y-1">
                               ...
                               <Button
                        className="fixed bottom-6 right-3 sm:bottom-4 sm:right-4 rounded-full w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-200 bg-blue-600 hover:bg-blue-700 border-0 cursor-pointer"
                        onClick={() => setOpen(true)}
                    >
